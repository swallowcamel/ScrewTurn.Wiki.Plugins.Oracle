...
namespace ScrewTurn.Wiki.Plugins.OracleCommon {
	...
	/// <summary>
	/// Implements a base class for all Oracle-based classes.
	/// </summary>
	public abstract class OracleClassBase {
        ...
        /// <param name="statementCount">statement count</param>
        /// <returns>The rows affected (-1 if the command failed).</returns>
        protected int ExecuteNonQuery(DbCommand command, bool close, bool isBatch, int statementCount)
        {
            return ExecuteNonQuery(command, close, true, isBatch, statementCount);
        }
		...
		protected int ExecuteNonQuery(DbCommand command, bool close, bool logError) {
            return ExecuteNonQuery(command, close, logError, false, -1);
		}
        ...
        /// <param name="close">A value indicating whether to close the connection after execution.</param>
        /// <param name="logError">A value indicating whether to log any error.</param>
        /// <param name="isBatch">is a batch script</param>
        /// <param name="batchCount">statement count </param>
        /// <returns></returns>
        protected int ExecuteNonQuery(DbCommand command, bool close, bool logError, bool isBatch, int statementCount)
        {
            int rows = -1;
            ...
            try
            {
                ...
                rows = isBatch ? statementCount : rows;
            }
            catch (DbException dbex)
            {
        ...
        /// <param name="scriptXml">script xml</param>
        protected void ExecuteScript(DbCommand cmd, string scriptXml)
        {
            XmlDocument scriptDoc = new XmlDocument();
            scriptDoc.LoadXml(scriptXml);
            XmlNodeList script_collections = scriptDoc.DocumentElement.ChildNodes;

            foreach (XmlNode script in script_collections)
            {
                try
                {
                    if (script.FirstChild == null || script.FirstChild.NodeType != XmlNodeType.CDATA) continue;
                    string cmdTxt = script.FirstChild.InnerText;
                    if (string.IsNullOrEmpty(cmdTxt)) continue;

                    CommandType cmdType = CommandType.Text;
                    XmlAttribute callType = script.Attributes["type"];
                    if (callType != null)
                    {
                        if (callType.Value == "proc") cmdType = CommandType.StoredProcedure;
                        if (callType.Value == "text") cmdType = CommandType.Text; ;
                    }
