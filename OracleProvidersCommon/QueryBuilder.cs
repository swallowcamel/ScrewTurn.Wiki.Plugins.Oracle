...
namespace ScrewTurn.Wiki.Plugins.OracleCommon {
	...
	public class QueryBuilder {
        ...
        /// <returns></returns>
        public string BuildBatchCommand(string query)
        {
            return string.Format("{0}{1}{2}{3}"
                , builder.GetType().GetProperty("BatchQueryStart").GetValue(builder, null)
                , query
                , builder.BatchQuerySeparator
                , builder.GetType().GetProperty("BatchQueryEnd").GetValue(builder, null)
                );
        }
		...
		public string SelectFrom(string table, string[] columns) {
			...
			for(int i = 0; i < columns.Length; i++) {
                OracleKeyWords.Filter(ref sb, columns[i], builder);
		...
		public string SelectFrom(string table, string joinedTable, string[] joinTableColumns, string[] joinJoinedTableColumns, Join join,
			...
			for(int i = 0; i < tableColumns.Length; i++) {
                OracleKeyWords.Filter(ref sb, table, builder);
				sb.Append(".");
                OracleKeyWords.Filter(ref sb,tableColumns[i], builder);

				sb.Append(" as ");
				
                OracleKeyWords.Filter(ref sb,string.Format("{0}_{1}",table,tableColumns[i]),builder);
			...
			for(int i = 0; i < joinedTableColumns.Length; i++) {
                OracleKeyWords.Filter(ref sb, joinedTable, builder);
				sb.Append(".");
                OracleKeyWords.Filter(ref sb,joinedTableColumns[i], builder);
				
				sb.Append(" as ");

                OracleKeyWords.Filter(ref sb, string.Format("{0}_{1}", joinedTable, joinedTableColumns[i]), builder);
			...
			for(int i = 0; i < joinTableColumns.Length; i++) {
                OracleKeyWords.Filter(ref sb, table, builder);
				sb.Append(".");
                OracleKeyWords.Filter(ref sb, joinTableColumns[i], builder);
				...
				sb.Append(".");
                OracleKeyWords.Filter(ref sb, joinJoinedTableColumns[i], builder);
		...
		public string SelectFrom(string table, string joinedTable, string tableColumn, string joinedTableColumn, Join join) {
            ...
            OracleKeyWords.Filter(ref sb, table, builder);
			sb.Append(".");
            OracleKeyWords.Filter(ref sb, tableColumn, builder);

			sb.Append(" = ");

            OracleKeyWords.Filter(ref sb, joinedTable, builder);
			sb.Append(".");
            OracleKeyWords.Filter(ref sb, joinedTableColumn, builder);
		...
		public string SelectFrom(string table, string joinedTable, string tableColumn, string joinedTableColumn, Join join,
			...
			for(int i = 0; i < otherJoinedTableColumns.Length; i++) {
                OracleKeyWords.Filter(ref sb,otherJoinedTable, builder);
				sb.Append(".");
                OracleKeyWords.Filter(ref sb, otherJoinedTableColumns[i], builder);

				sb.Append(" as ");

                OracleKeyWords.Filter(ref sb, string.Format("{0}_{1}", otherJoinedTable, otherJoinedTableColumns[i]), builder);
            ...
            OracleKeyWords.Filter(ref sb, otherJoinedTable, builder);
			sb.Append(".");
            OracleKeyWords.Filter(ref sb, otherJoinedTableColumn, builder);
		...
		private string BuildWhereInClause(string table, string column, string[] parameters) {
			...
			for(int i = 0; i < parameters.Length; i++) {
				if(builder.UseNamedParameters) {
					sb.Append(builder.ParameterNamePrefix);
					OracleKeyWords.Filter(ref sb,parameters[i],builder);
					sb.Append(builder.ParameterNameSuffix);
				}
				else {
		...
		public string AppendForBatch(string query, string secondQuery) {
			return string.IsNullOrEmpty(query.Trim())?secondQuery:query + builder.BatchQuerySeparator + secondQuery;
		}
