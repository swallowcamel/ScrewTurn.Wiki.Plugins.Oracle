...
namespace ScrewTurn.Wiki.Plugins.OracleCommon {
	...
	/// <summary>
	/// Implements a base class for a Oracle pages storage provider.
	/// </summary>
	public abstract class OraclePagesStorageProviderBase : OracleStorageProviderBase, IPagesStorageProviderV30 {
		...
		public new void Init(IHostV30 host, string config) {
			...
			index = new OracleIndex(new IndexConnector(GetWordFetcher, GetSize, GetCount, ClearIndex, DeleteDataForDocument, SaveDataForDocument, TryFindWord));
		}
		...
		private CategoryInfo GetCategory(DbTransaction transaction, string fullName) {
			...
			if(reader != null) {
				...
				while(reader.Read()) {
					if(result == null) result = new CategoryInfo(NameTools.GetFullName((reader["Category_Namespace"] as string).Trim(), reader["Category_Name"] as string), this);
		...
		private bool SetContent(DbTransaction transaction, PageContent content, int revision) {
			...
			if(content.Keywords.Length > 0) {
                ...
                fullQuery = queryBuilder.BuildBatchCommand(fullQuery);
		...
		private NavigationPath AddNavigationPath(DbTransaction transaction, string nspace, string name, PageInfo[] pages) {
            ...
            int rows = ExecuteNonQuery(command, false,true,count);
